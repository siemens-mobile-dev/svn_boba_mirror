...
__arm int PatchDynConst (int * p)
{
  ...
  int file;
  unsigned int iError;
  if ((file=_elfldr_fopen(elfldr_GetDir(DIR_ELFS_CONFIG | MEM_INTERNAL),(u16*)L"DYN_CONST.bin", A_ReadOnly+A_BIN, P_READ, &iError))<0) return (-101);	//не открываетс€ DYN_CONST.
  {
    FSTAT fstat;
...
__thumb long elfload(FILENAME filename, void *param1, void *param2, void *param3){
  ...
  if ((fin=elfldr_fopen(filename, A_ReadOnly+A_BIN, P_READ, &iError))<0) return -1;	//не открываетс€ ельф
