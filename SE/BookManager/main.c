...
void get_iconsID( MyBOOK* mbk )
{
  ...
  for ( i = 0;i < ICONS_COUNT;i++ )
  {
    if (wstrrchr(img_names[i],L'/'))
    {
      wchar_t * wstr_name = wstrrchr(img_names[i],L'/')+1;
      int path_len = (wstr_name-img_names[i]-1)/2;
  ...
  for ( i = 0;i < DIGITS_COUNT;i++ )
  {
    ...
    {
      wchar_t * wstr_name = wstrrchr(img_digits[i],L'/')+1;
      int path_len = (wstr_name-img_digits[i]-1)/2;
