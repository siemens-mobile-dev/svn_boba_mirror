...
char* GetCfgEdit()
{
  ...
  char *extname;
  if (isdir("4:\\ZBin\\etc", m_err) == 0)
  {
    extname = "0:\\ZBin\\etc\\extension.cfg";
  }
  else
  {
    extname = "4:\\ZBin\\etc\\extension.cfg";
  }
  unsigned int err ;
  int f;
  FSTATS fstat;
  ...
  if (GetFileStats(extname,&fstat,&err)<0) return ("");
  if ((f=fopen(extname,A_ReadOnly|A_BIN,P_READ,&err))==-1) return ("");
