...
#define MAX_ITEMS 10       // ћаксимальное количество пунктов меню
...
void Disp_Cont_Menu()
{
  ...
  if((CLAct_contact->subscription == SUB_BOTH)||(CLAct_contact->subscription==SUB_FROM))
   ...
   if((Act_contact->status == PRESENCE_SUBSCRIBE))//≈сли контакт запрвшивает подписку...
   {
...
/*
ќбработчик нажатий клавиш меню. ќтсюда следует вызывать необходимые действи€
...
void contc_menu_iconhndl(void *data, int curitem, void *unk)
{
  ...
  case CONTC_SUBSCRIBE:
    {
      strcpy(test_str,"«апросить подписку");
      break;
    }
  case CONTC_SUBSCRIBED:
    {
      strcpy(test_str,"–азрешить");
      break;
    }
  case CONTC_UNSUBSCRIBED:
    {
      strcpy(test_str,"ќтклонить");
      break;
    }
  case CONTC_UNSUBSCRIBE:
    {
      strcpy(test_str,"”далить подписку");
      break;
    }
  }
