...
__arm int DoUnknownFileType(WSHDR *filename)
{
  ...
  fname=fn+i; //”казатель на посл. байт
  while(fname[-1]!='.')
  {
    i--;
    if (i==0) {
  ...
  //“еперь fname указывает на расширение
  if ((f=fopen(extfile,A_ReadOnly,0,&ul))!=-1)
  {
	...
	if (c==':')
	{
          execname=s+i; //ѕоследний файл - универсальный
          sm--; //“олько первый вход
	}
      case 0xFFFE:
