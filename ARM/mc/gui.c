...
void InitScr()
{
  ...
  txt_h=GetFontYSIZE(FONT_SMALL); 
  if(!txt_h)
  {
    ShowMSG(1, (int)"Не уд. опред. выс. шрифта.\nGetFontYSIZE(SMALL_FONT)=0");
    txt_h=16; //!!!
  }
...
{ 
  ...
  if (ind==_CurIndex-_CurBase) 
  {
   ...
   {
    if(file->ws_size) 
    DrawString(file->ws_size,TXT_X,y2+ITM_B+1,ITM_X2-ITM_B-2,y2+ITM_B+txt_h,FONT_SMALL,TEXT_ALIGNLEFT,
                             (char*)&Colors[clInfoTxt],0);
   } 
   if(file->ws_attr)
   DrawString(file->ws_attr,TXT_X,y2+ITM_B+1,ITM_X2-ITM_B-2,y2+ITM_B+txt_h,FONT_SMALL,TEXT_ALIGNRIGHT,
                            (char*)&Colors[clInfoTxt],0);
  }
  ...
  if(file->attr & (FA_READONLY | FA_SYSTEM)) 
     tc=ind==_CurIndex-_CurBase?clSelFileReadOnly:clFileReadOnly; else
  if(file->attr & FA_HIDDEN )                
     tc=ind==_CurIndex-_CurBase?clSelFileHidden:clFileHidden; else
     tc=ind==_CurIndex-_CurBase?clSelFileNormal:clFileNormal;

  WSHDR* fn = file->uccnt?file->ws_short:file->ws_name;
  if(fn)
  DrawString(fn,TXT_X,y+ITM_B+1,ITM_X2-ITM_B-2,y+ITM_B+txt_h,FONT_SMALL,TEXT_ALIGNLEFT,
                       (char*)&Colors[tc],0);
}
...
void DrwName()
{
  if(!Busy && scfile && IsGuiOnTop(MAINGUI_ID))
  {
   ...
   {
     int gend=FALSE;
     int sc = getLVC(&scfile->ws_name->wsbody[sctpos+1], scfile->ws_name->wsbody[0]-sctpos);
     if(!sc)
     {
...
//     txt->wsbody[sc+1]=0;
     ...
     if(scfile->attr & (FA_READONLY | FA_SYSTEM)) 
       tc=clSelFileReadOnly; else
     if(scfile->attr & FA_HIDDEN )                
       tc=clSelFileHidden; else
       tc=clSelFileNormal;

     DrawRoundedFrame(TXT_X,y+ITM_B+1,ITM_X2-ITM_B-2,y+ITM_B+txt_h,0,0,1,0,(char*)&Colors[clSelItemBG]);
     DrawString(txt,TXT_X,y+ITM_B+1,ITM_X2-ITM_B-2,y+ITM_B+txt_h,FONT_SMALL,TEXT_ALIGNLEFT,
                          (char*)&Colors[tc],0);
     FreeWS(txt);
   }  
  } 
...
void DrwDrvAc(int ind)
{
  ...
  DrawString(guibuf,x+1,TOP_Y+TOP_B,x+DRV_W-2,TOP_Y+TOP_B+TOP_H-1,FONT_SMALL,TEXT_ALIGNMIDDLE+TEXT_OUTLINE,
...
void DrwCurTab()
{
  ...
  DrawString(guibuf,TAB_X+1,TOP_Y+TOP_B,TAB_X+TAB_W-2,TOP_Y+TOP_B+TOP_H-1,FONT_SMALL,TEXT_ALIGNMIDDLE+TEXT_OUTLINE,
                       (char*)&Colors[clTabTxt],(char*)&Colors[clTabTxtBD]);
}
...
void DrwIndex()
{
  ...
  if(chkcnt)
    wsprintf(guibuf,"%c  %d (%d)", s, chkcnt, _CurCount);
  else
    wsprintf(guibuf,"%c %d/%d", s, _CurIndex+1, _CurCount);
  DrawString(guibuf,IND_X1,TOP_Y+TOP_B,IND_X2,TOP_Y+TOP_B+TOP_H-1,FONT_SMALL,TEXT_OUTLINE,
...
void ShowProgr()
{
  ...
  if(progr_act)
  {
   ...
   DrawString(guibuf,PRGB_X1+2,PRG_AT_Y,PRGB_X2-2,PRG_AT_Y+txt_h,FONT_SMALL,TEXT_ALIGNMIDDLE+TEXT_OUTLINE,
                        (char*)&Colors[clProgrTxt],(char*)&Colors[clProgrTxtBD]);
  } 
  
  wsprintf(guibuf,"%d%% %d/%d",p,progr_cur,progr_max);
  DrawString(guibuf,PRGB_X1+2,PRG_PT_Y,PRGB_X2-2,PRG_PT_Y+txt_h,FONT_SMALL,TEXT_ALIGNMIDDLE+TEXT_OUTLINE,
                       (char*)&Colors[clProgrTxt],(char*)&Colors[clProgrTxtBD]);
