...
void UpdateFromOld(){
  ...
  for (int i=0;i<20;i++){
       ...
       memset(block5401+0x50*i+0x30,0x0,0x20);  
       block5401[0x50*i+0x30-1]=i;
  }
  EEFullReadBlock(5400,block5400,0,1024,0,0);

  for (int i=0;i<10;i++){
       memcpy(block5401+0x30*(i+1)+0x30,block5400+0x50*i+0x40,0x10);//loci
       memcpy(block5401+0x30*(i+1)+0x30+0x30,block5400+0x50*i+0x30,0x10);//smsc
       memcpy(block5401+0x30*(i+1)+0x30+0x4c,block5400+0x50*i+0xC,0x04);// profiles?
  }
  ...
  for (int i=0;i<10;i++){
       memcpy(block5401+0x30*(i+1)+0x30,block5400+0x50*i+0x0,0x10);//imsi
       memcpy(block5401+0x30*(i+1)+0x30+0x10,block5400+0x50*i+0x10,0x10);//ki 
       memcpy(block5401+0x30*(i+1)+0x30+0x20,block5400+0x50*i+0x20,0x20);//names
  }
  sprintf(block5401+0x31,"Physical");
