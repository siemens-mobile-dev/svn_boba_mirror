...
//  TOnStatusChange = procedure(Sender: TObject; UIN: String; Status: LongWord) of object;
  TOnStatusChange = procedure(Sender: TObject; UIN: String; Status: LongWord; XStatus:TXStatuses) of object;
  ...
  TOnXStatusTextResponse = procedure(Sender:TObject; UIN:LongWord; Title, Desc:string) of Object;
...
//  FOnDPktParse: TOnParseDirectPkt;
    ...
    FErrLang: TICQLangType;
    FXStatus: TXStatuses;
    FOnXStatusTextResponse: TOnXStatusTextResponse;
    procedure SetXStatus(const Value: TXStatuses);
...
//    Procedure OnSendFileDataProc(Sender:TObject; UIN:LongWord; Data:Pointer;Var DataLen:LongWord; Var IsLastPacket:Boolean);
  ...
  public
    ...
    destructor Destroy; override;
    procedure RequestXStatusText(UIN:LongWord);
    ...
    property XStatus: TXStatuses read FXStatus write SetXStatus default X_STATUS_NONE;
...
//  property OnPktDirectParse: TOnParseDirectPkt read FOnDPktParse write FOnDPktParse;
    ...
    property OnXStatusTextResponse: TOnXStatusTextResponse read FOnXStatusTextResponse write FOnXStatusTextResponse;
  end;
...
//      if ((pkt.Data[0]=0)and(pkt.Data[1]=142)) then
//        pkt.Len:=5;
      //SRV_COOKIE
...
T := GetInt(@pkt, 2);
      ...
      if T = 4 then
      begin
        OnIntError(nil, ERR_LOGIN, ICQLanguages[FErrLang].Translate(IMSG_EBAD_PASS)+' '+FData);
        FTOnConnectError(Self);
        Exit;
      end else
      if T = 8 then
      begin
        OnIntError(nil, ERR_LOGIN, ICQLanguages[FErrLang].Translate(IMSG_EOFTEN_LOGINS)+' '+FData);
        FTOnConnectError(Self);
        Exit;
      end else
      if T <> 5 then
...
{  PktInit(@Pkt, 2, FSeq);                         //Channel 2
  PktSnac(@Pkt, $13, $09, 0, 0);                 //Snac: Type x09/x05, ID x0000, Flags 0

  PktWStr(@Pkt,''); //Item name string
  PktInt(@Pkt,0,2); //Group ID#
  PktInt(@Pkt,$6871,2); //Item ID#
  PktInt(@Pkt,$04,2); //Type of item flag
  ...
  PktTLV(@tlv,$d2,$01,$01);
  PktTLV(@tlv,$d3,$01,$01);
  PktTLV(@tlv,$cb,$04,$ffffffff);

  PktInt(@Pkt,tlv.Len,2);
  PktAddArrBuf(@Pkt, @tlv, tlv.Len);
  ...
  case chan of
    ...
    begin
      ...
      begin
        ...
        c:=Pkt^.Len; //Displace of 05-01
        Inc(Pkt^.Len, 2); //	fragment identifier (array of required capabilities)
        Inc(Pkt^.Len, GetInt(Pkt, 2));          //Counts of following bytes + following bytes
        Move(Ptr(LongWord(Pkt) + c)^, chunks, Pkt^.Len-c);
        c:=Pkt^.Len-c;
...
{        PktInit(@ack_pkt, 2, FSeq);               //Channel 2
        ...
        Move(Ptr(LongWord(Pkt) + TSNACSZ)^, Ptr(LongWord(@ack_pkt) + ack_pkt.Len)^, 10); //Cookie und message chanel
        ...
        PktAddArrBuf(@ack_pkt, @chunks, c);      //First 47 bytes of source packet (with message)
        PktInt(@ack_pkt,$0101,2);
        PktInt(@ack_pkt,$0004,2);
        PktInt(@ack_pkt,$0000,2);
        PktInt(@ack_pkt,$FFFF,2);
        PktFinal(@ack_pkt);
...
{        if FDirect <> nil then
    ...
    begin
      s := GetWStr(Pkt);
{      If Pos(AIM_CAPS_ICQ, s) > 0 then                 //by BoBa
...
}      for XStatus := High(XStatusCLSIDs) downto Succ(Low(XStatusCLSIDs)) do
        if Pos(XStatusCLSIDs[XStatus],s) > 0 then break;
    end else
      Inc(Pkt^.Len, GetInt(Pkt, 2));
    end;
  end;
  if Assigned(OnStatusChange) then
    FOnStatusChange(Self, UIN, Status, XStatus);
...
//XStatus Text by BoBa 12.06.07
const
MGTYPE_XTRAZ_SCRIPT:string=#$3b#$60#$b3#$ef#$d8#$2a#$6c#$45#$a4#$e0#$9c#$5a#$5e#$67#$e8#$65;
MGTYPE_SCRIPT_NOTIFY=$08;

procedure TICQClient.RequestXStatusText(UIN: LongWord);
var pkt:TRawPkt;
    lpTLV2711: TRawPkt;
    lpTLV05: TRawPkt;
    ITime,IRandom:LongWord;
    body:string;
const Cap:string=          #$09#$46#$13#$49#$4C#$7F#$11#$D1#$82#$22#$44#$45#$53#$54#$00#$00;

begin
 if not LoggedIn then Exit;

 ITime:=GetTickCount();
 IRandom:=Random(High(Word));
 ...
 PktInt(@Pkt, IRandom, 4);                      //A seemingly random ID generated for each message.
 PktInt(@Pkt, 2, 2);                            //message channel - 2
 PktLStr(@Pkt, UIN);                            //The UIN to send the message to.

 PktInitRaw(@lpTLV05);                         //TLV(05)
 PktInt(@lpTLV05, $0000, 2);                   //message type, 0 - request
 PktInt(@lpTLV05, ITime, 4);                   //Seems to be a time stamp in 1/1500 sec since 8am of that Sunday.
 PktInt(@lpTLV05, IRandom, 4);                 //A seemingly random ID generated for each message.
 PktAddArrBuf(@lpTLV05, @Cap[1], 16);          //One of the capabilities sent in CLI_SETUSERINFO
 PktTLV(@lpTLV05, $000a, 2, 1);                //0x0001 - normal message 0x0002 - file ack or file ok
 ...
 PktInt(@lpTLV2711, $00000000, 4);            //plugin
 PktInt(@lpTLV2711, $00000000, 4);            //plugin
 PktInt(@lpTLV2711, $00000000, 4);            //plugin
 PktInt(@lpTLV2711, $00000000, 4);            //plugin
 ...
 PktInt(@lpTLV2711, FSeq, 2);                  //SEQ1
 PktInt(@lpTLV2711, $0e00, 2);                 //Unknown, seen: 0x1200 and 0x0e00.
 PktInt(@lpTLV2711, FSeq, 2);                  //SEQ1
 PktInt(@lpTLV2711, $00000000, 4);             //Unknown, always zero.
 PktInt(@lpTLV2711, $00000000, 4);             //Unknown, always zero.
 PktInt(@lpTLV2711, $00000000, 4);             //Unknown, always zero.
 PktInt(@lpTLV2711, $1a, 1);                   //0x01 - normal message, 0x1a - filerequest/response, contacts
 PktInt(@lpTLV2711, 0, 1);               //00 - normal message 80 - multiple recipients 03 - auto reply message request.
 PktLInt(@lpTLV2711, 0, 2); //word (LE) status code
...
//packPluginTypeId
 ...
 body:='<N><QUERY>&lt;Q&gt;&lt;PluginID&gt;srvMng&lt;/PluginID&gt;&lt;/Q&gt;</QUERY>'+
'<NOTIFY>&lt;srv&gt;&lt;id&gt;cAwaySrv&lt;/id&gt;&lt;req&gt;&lt;id&gt;AwayStat&lt;/id&gt;'+
'&lt;trans&gt;6&lt;/trans&gt;&lt;senderId&gt;'+inttostr(FLUIN)+
'&lt;/senderId&gt;&lt;/req&gt;&lt;/srv&gt;</NOTIFY></N>'#13#10;

 PktLInt(@lpTLV2711, $4f,2);                  // Length
 PktStr(@lpTLV2711,  MGTYPE_XTRAZ_SCRIPT);    // Message Type GUID
 PktLInt(@lpTLV2711, MGTYPE_SCRIPT_NOTIFY,2); // Function ID
 PktLInt(@lpTLV2711, $002a, 4);               // Request type string
 PktStr(@lpTLV2711, 'Script Plug-in: Remote Notification Arrive');

 PktInt(@lpTLV2711, $00000100,4);            // Unknown binary stuff
 PktInt(@lpTLV2711, $00000000,4);
 PktInt(@lpTLV2711, $00000000,4);
 PktInt(@lpTLV2711, $0000,2);
 PktInt(@lpTLV2711, $00,1);

 PktLInt(@lpTLV2711,length(body)+4,4);
 PktDWStr(@lpTLV2711,body);
...
////plugin msg (XStatus) add by BoBa 14.06.07
  if MsgType=$1A then
   begin
    RetMsg := GetLNTS(Pkt);
    Inc(Pkt^.Len, 2);                  // Length
    if GetStr(Pkt,16)<>MGTYPE_XTRAZ_SCRIPT then exit;  // Message Type GUID
    if GetLInt(Pkt,2)<>MGTYPE_SCRIPT_NOTIFY then exit; // Function ID
    if GetLInt(Pkt,4)<>$002a then exit; // Request type string
    if GetStr(Pkt, $2a)<>'Script Plug-in: Remote Notification Arrive' then exit;
    Inc(Pkt^.Len, 19);
    body:=UTF8ToStr(GetDWStr(Pkt));
    p1:=pos(title1,body)+length(title1);
    p2:=posex(title2,body,p1)-p1;
//    inc(p2);
    title:=midstr(body,p1,p2);
    p1:=pos(desc1,body)+length(desc1);
    p2:=posex(desc2,body,p1)-p1;
//    inc(p2);
    desc:=midstr(body,p1,p2);
    if Assigned(OnXStatusTextResponse) then
      OnXStatusTextResponse(Self, strtoint(FUIN), title, desc);
    Exit;
   end;
...
//////////////////XStatus by BoBa 12.06.07
procedure TICQClient.SetXStatus(const Value: TXStatuses);
var pkt:TRawPkt;
begin
 if FXStatus=Value then Exit;
 FXStatus := Value;
 if not LoggedIn then Exit;
